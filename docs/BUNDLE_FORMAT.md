# `.companion` バンドル仕様 v0（過去の草稿）

> **Status:** 現行の authoritative なスキーマは [`SCHEMA.md`](SCHEMA.md)（`companion-spec/0.1`）。
> 本ファイルはプロトタイプ期（`prototypes/companion-desktop/`）に書かれた最初のスケッチを記録として残したもの。
> 実装・議論では `SCHEMA.md` を参照すること。本ファイルは次のいずれかで役目を終える：
> (1) プロトタイプの退役時に削除、(2) v0 → v0.1 移行ノートに書き換え。
